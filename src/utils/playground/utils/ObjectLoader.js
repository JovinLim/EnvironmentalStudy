...
export default class ObjectLoader {
  ...
  async asyncPause() {
    ...
    if (Date.now() - this.lastAsyncPause >= 100) {
      this.lastAsyncPause = Date.now();
      this.existingAsyncPause = new Promise((resolve) =>
      ...
      if (Date.now() - this.lastAsyncPause > 500)
        console.log(
          "Loader Event loop lag: ",
          Date.now() - this.lastAsyncPause
        );
    }
  }
  ...
  async traverseAndConstruct(obj, onProgress) {
    ...
    if (Array.isArray(obj) && obj.length !== 0) {
      ...
      for (let element of obj) {
        if (typeof element !== "object" && !this.options.fullyTraverseArrays)
          return obj;
  ...
  async *getRawObjectIterator() {
    ...
    if (childrenIds.length > 50) {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        if (i < 3)
          nextCachePromise = this.cacheGetObjects(splitBeforeCacheCheck[i + 1]);
  ...
  async getRawRootObject() {
    ...
    const response = await fetch(this.requestUrlRootObj, {
