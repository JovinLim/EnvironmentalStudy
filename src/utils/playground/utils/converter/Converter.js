...
import MeshTriangulationHelper from "./MeshTriangulationHelper.js";
...
export default class Converter {
  ...
  async traverseAndConvert(obj, callback, scale = true, parents = []) {
    ...
    if (this[`${type}ToBufferGeometry`]) {
      try {
        await callback(
          await this[`${type}ToBufferGeometry`](obj.data || obj, scale)
        );
        return;
      } catch (e) {
        console.warn(
  ...
  async BrepToBufferGeometry(obj, scale = true) {
    try {
      ...
      let { bufferGeometry } = await this.MeshToBufferGeometry(
        await this.resolveReference(obj.displayValue || obj.displayMesh),
        scale
      );
  ...
  async PolycurveToBufferGeometry(object, scale = true) {
    ...
    let geometry = mergeBufferGeometries(buffers);
